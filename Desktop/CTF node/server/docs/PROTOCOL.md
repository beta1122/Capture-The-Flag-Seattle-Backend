...
is [`src/ctf/protocol.ts`](../src/ctf/protocol.ts); this document is a
...
- If the match is `inGame` or `postGame`, you get a `gameUpdate`, immediately followed by a
  `playerState` - a private snapshot sent only to that player's own socket, covering the
  bits of state that are otherwise only ever delivered as one-shot `*Result` responses
  (veto/invisibility countdowns, an in-progress challenge, double-up status, coins). Without
  it, a client that lost its in-memory state (e.g. an app relaunch mid-match) has no way to
  recover that state short of re-deriving it from the next action's result - the veto/
  invisibility timers in particular are still enforced server-side either way, so the
  countdown UI just goes missing and the player hits unexplained rejections until it expires.

A client must be able to handle either `preGameUpdate` or `gameUpdate`+`playerState` arriving
as the very first thing after connecting - don't assume it's always `preGameUpdate` just
because that's what happens on a fresh join before anyone starts the game.
...
| `playerState` | private per-player snapshot: `{ coins, vetoPeriodEnd?, invisibilityEnd?, activeChallenge?, doubledUp }` | *(new)* - sent only to that player's own socket, immediately after `gameUpdate`, on any (re)connect once the match is `inGame` or `postGame` |
...
endpoint exists yet) - it's for replay/debugging. See `src/ctf/persistence/` if you need to
