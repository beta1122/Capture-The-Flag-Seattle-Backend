...
// Private per-player snapshot, sent only to that player's own socket on connect/reconnect.
// Mirrors the state that's otherwise only ever delivered as one-shot *Result responses, so
// a client that lost its in-memory state (e.g. app relaunch) can rebuild it.
