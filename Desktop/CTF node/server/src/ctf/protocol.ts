...
import {
    GameUpdate, PreGameUpdate, Team, Challenge, PlayerState,
...
export type CTFServerMessage =
  ...
  // Private per-player state snapshot. Sent only to the connecting player's own socket,
  // alongside the gameUpdate, on any (re)connect that happens once the match is inGame or
  // postGame - the resync path for state that's otherwise only ever delivered in one-shot
  // *Result responses (veto/invisibility timers, an in-progress challenge, coins, ...).
