...
import { Error, isError } from "../../common/types";
import { Challenge, HandleTagResponse, PlayerState, Team, UpdateCoinsResponse, UseInvisPotResponse, VetoChallengeResponse } from "../types";
...
export class PlayerManager{
    ...
    async handleTag(taggerName: string): Promise<Error|HandleTagResponse>{
        ...
        if(gameContext.gameState != "inGame"){
            return {errorCode: 400, description: "Not in game while trying to handle tag"};
        }
        let coinsOwed =0;
    ...
    async useInvisPot(): Promise<Error|UseInvisPotResponse>{
        const gameContext = this.getGameContext()
        if(gameContext.gameState != "inGame"){
            return {errorCode: 400, description: "Not in game while trying to useInvisPot"};
        }
    ...
    // Snapshot of this player's private state, for the playerState resync message sent on
    // (re)connect. Not locked - read-only, and called from the connect path where a torn
    // read is a non-issue (worst case it's one tick stale, same as any other snapshot).
    getPlayerState(): PlayerState {
        return {
            coins: this.coins,
            vetoPeriodEnd: this.vetoPeriodEnd !== undefined ? new Date(this.vetoPeriodEnd).toISOString() : undefined,
            invisibilityEnd: this.invisibilityEnd !== undefined ? new Date(this.invisibilityEnd).toISOString() : undefined,
            activeChallenge: this.currChallenge,
            doubledUp: this.doubleNextChallenge,
        };
    }
}