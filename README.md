...
- **Own wire protocol**: plain JSON messages over `ws`, no external multiplayer
  ...
  [`src/rooms/ctf/protocol.ts`](src/rooms/ctf/protocol.ts)) is what a mobile client
...
- **Generic room layer**: `src/rooms/GameRoom.ts` is a game-agnostic base class (WebSocket
  ...
  under `src/rooms/ctf/`. A future second game mode would extend `GameRoom` without
...
4. Send/receive JSON text frames shaped `{ "type": "...", "payload": {...} }` per the
   ...
   (Swift) or `data class` (Kotlin) mirroring [`src/rooms/ctf/protocol.ts`](src/rooms/ctf/protocol.ts) — that file is the source of truth if this doc and the code ever drift.
